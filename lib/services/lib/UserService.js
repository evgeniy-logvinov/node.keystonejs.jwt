...
class UserService extends AbstractService {
	...
	create({ email, password, confirmPassword, emailPreferences = false, phone, name }) {
		if (!email) {
		...
		return User.create({ email, password, phone, emailPreferences, name: { first: name.first, last: name.last } })
	}
}
