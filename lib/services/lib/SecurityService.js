...
const { WebError, ValidationError, UnauthorizedAccessError } = requireRoot('lib/errors');
const { checkEmail, checkPhone, checkName } = requireRoot('lib/utility');
...
module.exports = class SecurityService {
	...
	getLoginResponse(token, refreshToken) {
		return {
			...
			refreshExpiredLife: process.env.JWT_REFRESH_TOKEN_LIFE,
